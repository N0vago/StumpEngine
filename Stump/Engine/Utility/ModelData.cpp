...
{
	std::string text = Shader::GetFileContent(filePath);
...
Mesh ModelData::LoadMesh(unsigned int indMesh)
{
	...
	std::vector<Math::Vector3> positions = GroupFloatsVec3(posVec);
	std::vector<float> normalVec = GetFloats(JSON["accessors"][normalAccInd]);
	std::vector<Math::Vector3> normals = GroupFloatsVec3(normalVec);
...
void ModelData::TraverseNode(unsigned int nextNode, Matrix3x4 matrix)
{
	...
	if (node.find("translation") != node.end())
	{
		...
		translation = Math::Vector3(transValues[0], transValues[1], transValues[2]);
	}
	...
	if (node.find("scale") != node.end())
	{
		...
		scale = Math::Vector3(scaleValues[0], scaleValues[1], scaleValues[2]);
	}
...
std::vector<unsigned char> ModelData::GetData()
{
	...
	bytesText = Shader::GetFileContent((fileDirectory + uri).c_str());
...
std::vector<Texture> ModelData::GetTextures()
{
	...
	for (unsigned int i = 0; i < JSON["images"].size(); i++)
	{
		...
		if (!skip)
		{
			...
			{
				Texture diffuse = Texture((fileDirectory + texPath).c_str(), TextureType::Diffuse, loadedTex.size());
			...
			{
				Texture specular = Texture((fileDirectory + texPath).c_str(), TextureType::Specular, loadedTex.size());
...
std::vector<Math::Vector3> ModelData::GroupFloatsVec3(std::vector<float> floatVec)
{
	...
	for (unsigned int i = 0; i < floatVec.size(); i += floatsPerVector)
	{
		vectors.push_back(Math::Vector3(0, 0, 0));
