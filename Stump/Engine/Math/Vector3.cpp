...
namespace Math {
	...
	{
		x = MathF::Stepify(x, p_val.x);
		y = MathF::Stepify(y, p_val.y);
		z = MathF::Stepify(z, p_val.z);
	}
	...
	{
		return IsEqualApprox(p_v, MathF::EPSILON);
	}

	bool Vector3::IsEqualApprox(const Vector3& p_v, num_fd p_tolerance) const
	{
		return MathF::IsEqualApprox(x, p_v.x, p_tolerance) &&
			MathF::IsEqualApprox(y, p_v.y, p_tolerance) &&
			MathF::IsEqualApprox(z, p_v.z, p_tolerance);
	}
	...
	{
		return MathF::Absf(Length() - 1.0f) < 1e-6f;
	}
	...
	Vector3 Vector3::Slerp(const Vector3& p_to, num_fd p_weight) const
	{
		...
		num_fd dot_product = MathF::Clampf(this->Dot(p_to), -1.0f, 1.0f);
		num_fd theta = MathF::Acosf(dot_product) * p_weight;
		// Calculate the relative vector
		Vector3 relative_vec = (p_to - (*this) * dot_product).Normalized();
		// Calculate the final vector
		return ((*this) * MathF::Cosf(theta)) + (relative_vec * MathF::Sinf(theta));
	}
	...
	{
		return Vector3(MathF::Absf(x), MathF::Absf(y), MathF::Absf(z));
	}

	Vector3 Vector3::Floor() const
	{
		return Vector3(MathF::Floorf(x), MathF::Floorf(y), MathF::Floorf(z));
	}
	...
	{
		return Vector3(MathF::Ceilf(x), MathF::Ceilf(y), MathF::Ceilf(z));
	}

	Vector3 Vector3::Round() const
	{
		return Vector3(MathF::Roundf(x), MathF::Roundf(y), MathF::Roundf(z));
	}
	...
	{
		return Vector3(
			MathF::Fposmod(x, p_mod),
			MathF::Fposmod(y, p_mod),
			MathF::Fposmod(z, p_mod)
		);
	}
	...
	{
		return Vector3(
			MathF::Fposmod(x, p_modv.x),
			MathF::Fposmod(y, p_modv.y),
			MathF::Fposmod(z, p_modv.z)
		);
	}
	...
	{
		num_fd dot_product = MathF::Clampf(this->Dot(p_to) / (this->Length() * p_to.Length()), -1.0f, 1.0f);
		return MathF::Acosf(dot_product);
	}

	num_fd Vector3::SignedAngleTo(const Vector3& p_to, const Vector3& p_axis) const
	{
		Vector3 cross_to = Cross(p_to);
		num_fd unsigned_angle = MathF::Atan2(cross_to.Length(), Dot(p_to));
	...
	{
		num_fd theta = MathF::ToRadiansf(p_degrees);

		num_fd cos_angle = MathF::Cosf(theta);
		num_fd sin_angle = MathF::Sinf(theta);
