...
namespace Math {
	...
	void Matrix4x4::SetPerspective(num_fd p_fovy_degrees, num_fd p_aspect, num_fd p_near, num_fd p_far, bool p_flip_fov) {
		...
		num_fd radians = p_fovy_degrees / 2.0 * MathF::PI / 180.0;
	...
	void Matrix4x4::Invert() {
		...
		for (k = 0; k < 4; k++) {
			...
			for (i = k; i < 4; i++) {
				for (j = k; j < 4; j++) {
					if (MathF::Abs(matrix[i][j]) > MathF::Abs(pvt_val)) {
						pvt_i[k] = i;
						pvt_j[k] = j;
						pvt_val = matrix[i][j];
					}
			...
			determinat *= pvt_val;
			if (MathF::Abs(determinat) < (num_fd)1e-7) {
	...
	num_fd Matrix4x4::GetFOV() const {
		...
		if ((matrix[8] == 0) && (matrix[9] == 0)) {
			return MathF::ToRadians(MathF::Acos(MathF::Abs(right_plane.normal.x))) * 2;
		}
		else {
			...
			return MathF::ToRadians(MathF::Acos(MathF::Abs(left_plane.normal.x))) + MathF::ToRadians(MathF::Acos(MathF::Abs(right_plane.normal.x)));
		}
	}
