...
namespace Math {
	...
	{
		return Vector2(MathF::Absf(x), MathF::Absf(y));
	}

	Vector2 Vector2::Floor() const
	{
		return Vector2(MathF::Floorf(x), MathF::Floorf(y));
	}
	...
	{
		return Vector2(MathF::Ceilf(x), MathF::Ceilf(y));
	}

	Vector2 Vector2::Round() const
	{
		return Vector2(MathF::Roundf(x), MathF::Roundf(y));
	}
	...
	{
		return MathF::Atan2f(y, x);
	}

	num_fd Vector2::AngleTo(const Vector2& p_to) const
	{
		return MathF::Atan2f(Cross(p_to), Dot(p_to));
	}

	num_fd Vector2::AngleToPoint(const Vector2& p_to) const
	{
		return MathF::Atan2f(y - p_to.y, x - p_to.x);
	}

	void Vector2::SetRotation(num_fd p_radians)
	{
		x = MathF::Cosf(p_radians);
		y = MathF::Sinf(p_radians);
	}

	void Vector2::Rotate(num_fd p_radians)
	{
		num_fd cos_theta = MathF::Cosf(p_radians);
		num_fd sin_theta = MathF::Sinf(p_radians);
