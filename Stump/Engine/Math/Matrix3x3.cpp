...
namespace Math{
	...
	void Matrix3x3::FromZ(const Vector3& p_z) {
		if (MathF::Abs(p_z.z) > (num_fd)MathF::SQRT12) {
			num_fd a = p_z[1] * p_z[1] + p_z[2] * p_z[2];
			num_fd k = 1 / MathF::Sqrt(a);
	...
	bool Matrix3x3::IsDiagonal() const {
		return (
			MathF::IsZeroApprox(elements[0][1]) && MathF::IsZeroApprox(elements[0][2]) &&
			MathF::IsZeroApprox(elements[1][0]) && MathF::IsZeroApprox(elements[1][2]) &&
			MathF::IsZeroApprox(elements[2][0]) && MathF::IsZeroApprox(elements[2][1]));
	}

	bool Matrix3x3::IsRotation() const {
		return MathF::IsEqualApprox(Determinant(), 1, (num_fd)MathF::UNIT_EPSILON) && IsOrthogonal();
	}

	bool Matrix3x3::IsSymmetric() const {
		if (!MathF::IsEqualApproxRatio(elements[0][1], elements[1][0], (num_fd)MathF::UNIT_EPSILON)) {
			return false;
		}
		if (!MathF::IsEqualApproxRatio(elements[0][2], elements[2][0], (num_fd)MathF::UNIT_EPSILON)) {
			return false;
		}
		if (!MathF::IsEqualApproxRatio(elements[1][2], elements[2][1], (num_fd)MathF::UNIT_EPSILON)) {
			return false;
		}
	...
	Matrix3x3 Matrix3x3::Diagonalize() {
		...
		int ite = 0;
		Matrix3x3 acc_rot;
		while (off_matrix_norm_2 > (num_fd)MathF::EPSILON && ite++ < ite_max) {
			...
			num_fd angle;
			if (MathF::IsEqualApprox(elements[j][j], elements[i][i])) {
				angle = MathF::PI / 4;
			}
			else {
				angle = 0.5f * MathF::Atan(2 * elements[i][j] / (elements[j][j] - elements[i][i]));
			}

			// Compute the rotation matrix
			Matrix3x3 rot;
			rot.elements[i][i] = rot.elements[j][j] = MathF::Cos(angle);
			rot.elements[i][j] = -(rot.elements[j][i] = MathF::Sin(angle));
	...
	Vector3 Matrix3x3::GetEulerXYZ() const {
		...
		if (sy < (1 - (num_fd)MathF::EPSILON)) {
			if (sy > -(1 - (num_fd)MathF::EPSILON)) {
			
				if (elements[1][0] == 0 && elements[0][1] == 0 && elements[1][2] == 0 && elements[2][1] == 0 && elements[1][1] == 1) {
				
					euler.x = 0;
					euler.y = MathF::Atan2(elements[0][2], elements[0][0]);
					euler.z = 0;
				}
				else {
					euler.x = MathF::Atan2(-elements[1][2], elements[2][2]);
					euler.y = MathF::Asin(sy);
					euler.z = MathF::Atan2(-elements[0][1], elements[0][0]);
				}
			}
			else {
				euler.x = MathF::Atan2(elements[2][1], elements[1][1]);
				euler.y = -MathF::PI / 2.0;
				euler.z = 0.0;
			}
		}
		else {
	...
	void Matrix3x3::SetEulerXYZ(const Vector3& p_euler) {
		...
		c = MathF::Cos(p_euler.x);
		s = MathF::Sin(p_euler.x);
		Matrix3x3 xmat(1, 0, 0, 0, c, -s, 0, s, c);

		c = MathF::Cos(p_euler.y);
		s = MathF::Sin(p_euler.y);
		Matrix3x3 ymat(c, 0, s, 0, 1, 0, -s, 0, c);

		c = MathF::Cos(p_euler.z);
		s = MathF::Sin(p_euler.z);
	...
	Vector3 Matrix3x3::GetEulerYZX() const {
		...
		if (sz < (1 - (num_fd)MathF::EPSILON)) {
			if (sz > -(1 - (num_fd)MathF::EPSILON)) {
				euler.x = MathF::Atan2(-elements[1][2], elements[1][1]);
				euler.y = MathF::Atan2(-elements[2][0], elements[0][0]);
				euler.z = MathF::Asin(sz);
			}
			else {
				euler.x = MathF::Atan2(elements[2][1], elements[2][2]);
				euler.y = 0.0;
				euler.z = -MathF::PI / 2.0;
			}
		}
	...
	Vector3 Matrix3x3::GetEulerYXZ() const {
		...
		if (m12 < (1 - (num_fd)MathF::EPSILON)) {
			if (m12 > -(1 - (num_fd)MathF::EPSILON)) {
			
				if (elements[1][0] == 0 && elements[0][1] == 0 && elements[0][2] == 0 && elements[2][0] == 0 && elements[0][0] == 1) {
				
					euler.x = MathF::Atan2(-m12, elements[1][1]);
					euler.y = 0;
					euler.z = 0;
				}
				else {
					euler.x = MathF::Asin(-m12);
					euler.y = MathF::Atan2(elements[0][2], elements[2][2]);
					euler.z = MathF::Atan2(elements[1][0], elements[1][1]);
				}
			}
			else {
				euler.x = MathF::PI * 0.5;
				euler.y = MathF::Atan2(elements[0][1], elements[0][0]);
				euler.z = 0;
			}
		}
		else {
	...
	Vector3 Matrix3x3::GetEulerZXY() const {
		...
		if (sx < (1 - (num_fd)MathF::EPSILON)) {
			if (sx > -(1 - (num_fd)MathF::EPSILON)) {
				euler.x = MathF::Asin(sx);
				euler.y = MathF::Atan2(-elements[2][0], elements[2][2]);
				euler.z = MathF::Atan2(-elements[0][1], elements[1][1]);
			}
			else {
				euler.x = -MathF::PI / 2.0;
				euler.y = MathF::Atan2(elements[0][2], elements[0][0]);
				euler.z = 0;
			}
		}
	...
	Vector3 Matrix3x3::GetEulerZYX() const {
		...
		if (sy < (1 - (num_fd)MathF::EPSILON)) {
			if (sy > -(1 - (num_fd)MathF::EPSILON)) {
				euler.x = MathF::Atan2(elements[2][1], elements[2][2]);
				euler.y = MathF::Asin(-sy);
				euler.z = MathF::Atan2(elements[1][0], elements[0][0]);
			}
			else {
				euler.x = 0;
				euler.y = MathF::PI / 2.0;
	...
	bool Matrix3x3::IsEqualApproxRatio(const Matrix3x3& p_a, const Matrix3x3& p_b, num_fd p_epsilon) const {
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
				if (!MathF::IsEqualApproxRatio(p_a.elements[i][j], p_b.elements[i][j], p_epsilon)) {
					return false;
				}
			}
		}
	...
	Quaternion Matrix3x3::GetQuat() const {
		...
		if (trace > 0) {
			...
			num_fd s = MathF::Sqrt(m.elements[i][i] - m.elements[j][j] - m.elements[k][k] + 1);
			temp[i] = s * 0.5f;
			s = 0.5f / s;
	...
	void Matrix3x3::GetAxisAngle(Vector3& r_axis, num_fd& r_angle) const {
		...
		num_fd x, y, z;
		if (MathF::IsZeroApprox(elements[0][1] - elements[1][0]) && MathF::IsZeroApprox(elements[0][2] - elements[2][0]) && MathF::IsZeroApprox(elements[1][2] - elements[2][1])) {
			// Singularity found.
			// First check for identity matrix which must have +1 for all terms in leading diagonal and zero in other terms.
			if (IsDiagonal() && (MathF::Abs(elements[0][0] + elements[1][1] + elements[2][2] - 3) < 3 * MathF::EPSILON)) {
		...
		double s = MathF::Sqrt((elements[2][1] - elements[1][2]) * (elements[2][1] - elements[1][2]) + (elements[0][2] - elements[2][0]) * (elements[0][2] - elements[2][0]) + (elements[1][0] - elements[0][1]) * (elements[1][0] - elements[0][1])); // Used to normalise.
		...
		r_axis = Vector3(x, y, z);
		// acos does clamping.
		r_angle = MathF::Acos((elements[0][0] + elements[1][1] + elements[2][2] - 1) / 2);
	}
	...
	void Matrix3x3::SetAxisAngle(const Vector3& p_axis, num_fd p_angle) {
		...
		num_fd cosine = MathF::Cos(p_angle);
		num_fd sine = MathF::Sin(p_angle);
		num_fd t = 1 - cosine;
	...
	Matrix3x3 Matrix3x3::Slerp(const Matrix3x3& p_to, const num_fd& p_weight) const {
		...
		b.elements[0] *= MathF::Lerp(elements[0].Length(), p_to.elements[0].Length(), p_weight);
		b.elements[1] *= MathF::Lerp(elements[1].Length(), p_to.elements[1].Length(), p_weight);
		b.elements[2] *= MathF::Lerp(elements[2].Length(), p_to.elements[2].Length(), p_weight);
