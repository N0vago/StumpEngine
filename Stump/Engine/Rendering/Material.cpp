...
#include <cstdint>
namespace Rendering {
    void Material::SetTexture(std::shared_ptr<Texture> p_texture)
    {
        textures[p_texture->type] = std::move(p_texture);
    }
    void Material::Bind() const
    {
        ...
        for (const auto& [name, uniform] : uniforms)
        {
            ...
            case UniformType::Vec3:
                shader->SetVec3(
                    name.c_str(),
                    std::get<Math::Vector3>(uniform.data)
                );
                break;
    ...
    void Material::SetVec3(const std::string& r_name, Math::Vector3& r_value)
    {
